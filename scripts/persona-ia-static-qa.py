...
require("return trimmed" in persona_tag, "Pending AI prompt sanitizer must preserve queries outside the active persona")
...
require("var visibleCategories: [SearchCategory]" in search_vm and "SearchCategory.allCases" in search_vm, "Search does not expose every category independently of profile")
require("guard inferredCategory.isVisible(for: activePersona)" not in search_vm, "Search query inference still blocks cross-profile categories")
...
require("visibleSavedItems" in favorites_view and "savedStore.savedItems" in favorites_view, "FavoritesView must expose every saved item regardless of persona")
