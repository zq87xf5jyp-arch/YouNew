...
import { validateBusinessInquiryPayload } from "./validation.ts";
...
Deno.serve(async (request) => {
  ...
  const networkKey = forwardedFor || connectingIp || "unavailable";
  const rateKey = await sha256Hex(`${rateLimitSalt}:${networkKey}`);
  ...
  if (error) {
    ...
    if (
      ...
      error.message.includes("kvk_required") ||
      error.message.includes("invalid_selections") ||
      error.message.includes("bot_rejected")
    ) {
  ...
  if (
    ...
    !/^YNI-[A-F0-9]{12}$/.test(receipt.confirmation_code) ||
