  ...
  function journeyCard(journey: JourneyView) {
    ...
    return (
      <section className="journey-card" id={journey.id} key={journey.id} aria-labelledby={`${journey.id}-title`}>
        ...
        ) : <div className="journey-unavailable"><p>This sequence remains unavailable until its guides are reviewed and published.</p><Link href="/search">Search published content</Link></div>}
      </section>
    );
  }
