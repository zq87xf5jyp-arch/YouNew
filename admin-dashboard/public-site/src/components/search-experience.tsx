    ...
    <div className="search-experience">
      ...
      {!loading && !loadError && (submittedQuery || hasActiveFilters) ? (
        <section className="search-results" aria-labelledby="results-title">
          ...
          {ranked.length ? <div className="search-result-list">{ranked.map(({ document }) => <article key={document.id}><Link href={document.route}><span>{document.type}{document.city ? ` · ${document.city}` : ""}</span><h3>{document.title}</h3><p>{document.summary}</p></Link><SaveButton item={{ id: document.id, route: document.route, title: document.title, kind: document.type }} compact /></article>)}</div> : <div className="empty-state"><Search aria-hidden /><h2>No published match</h2><p>Try a shorter term, clear one of the filters, or browse categories. Search only includes published and source-checked content.</p><button className="button button-outline" type="button" onClick={() => { setFilters(emptyFilters); syncUrl(submittedQuery, emptyFilters); }}>Clear filters</button></div>}
        </section>
      ) : null}
    </div>
  );
