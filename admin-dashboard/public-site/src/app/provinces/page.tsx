  ...
  return (
    <PageShell>
      <section className="app-hero section-shell compact-hero">
        ...
        <p>Browse the Dutch provinces that currently contain released YouNew city, organization, place or guide records. Coverage reflects the production dataset and does not imply nationwide completeness.</p>
        <div className="dataset-note"><strong>{provinces.length}</strong> provinces · <strong>{cities.length}</strong> cities · <strong>{entities.length}</strong> released records</div>
      </section>
      ...
      <section className="section-shell coverage-guidance" aria-labelledby="province-use-title">
        <div className="listing-heading"><div><span>Use the released coverage</span><h2 id="province-use-title">Choose the most useful route</h2></div></div>
        <div className="information-grid">
          ...
          <article><Map aria-hidden /><h3>Compare locations</h3><p>Use the accessible coverage map to filter released records by city, province, category and content type.</p><Link href="/map">Open the map <ArrowRight aria-hidden /></Link></article>
