...
export default async function CategoryDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <PageShell><section className="app-hero section-shell compact-hero"><Breadcrumbs items={[{ label: "Categories", href: "/categories" }, { label: category.title }]} /><h1>{category.title}</h1><p>{category.summary}</p><div className="dataset-note"><strong>{entities.length}</strong> published items across {category.entityTypes.join(", ")}</div></section><section className="section-shell app-content-block"><EntityListing entities={entities} viewAllHref={`/search?category=${category.slug}`} /></section></PageShell>;
}
