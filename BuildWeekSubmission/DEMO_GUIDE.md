...
Media-rights status updated: 22 July 2026. The shipped app asset catalog passes
its deterministic rights gate; recording and screenshot files remain a separate
release-media scope.
...
- Use the catalog inventory and distribution conditions in
  [MEDIA_RIGHTS_AND_ATTRIBUTION.md](MEDIA_RIGHTS_AND_ATTRIBUTION.md). The catalog
  has 170 governed assets and zero unresolved records.
- Prepare the exact credit lines for every reproduced licensed photograph from
  `YouNew/Resources/MediaAttributions.json` for the video description/end card.
...
| 11 | Tap **Map** again, choose Noord-Holland, then select **Amsterdam** from the exposed marker or city control. Hold on the Amsterdam hero/name. If a flag or coat of arms is visible, use it only for informational identification and do not imply municipality endorsement. | An Amsterdam city surface/detail opens. Amsterdam is one of the five governed `cities-v0.1.0` records. | `map.city.amsterdam`; `cities-v0.1.0` release manifest; `BuildWeekFinal/MEDIA_RIGHTS_FINAL.md`. |
...
- Add the required credit lines and modification notices for every reproduced
  licensed photograph to the video description or end card.
- Review screenshot 08 as a separate release-media artifact before promotional
  use; the catalog gate does not inventory screenshot files.
