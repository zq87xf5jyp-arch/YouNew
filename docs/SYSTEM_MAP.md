...
| iOS app | Native YouNew product | `YouNew/` and `YouNew.xcodeproj` | Apple distribution | Apple signing and App Store controls | Version 1.1 is listed in the Netherlands App Store; version 1.2 (7) has a clean arm64 archive, Apple Distribution export and accepted App Store Connect upload |
| Public and business website | Guides, search, journeys, map, support and reviewed commercial inquiry | `admin-dashboard/public-site/` plus generated DataProject projection | OpenAI Sites / Cloudflare custom-domain runtime at `https://younew.nl`; Hostinger package is a standby artifact | Static CSP/headers; public writes only through controlled Edge Functions | Candidate builds 582 static routes with 572 indexable URLs; 108 tests, link, smoke, security and predeploy gates pass |
...
| GitHub | Version control and CI | `https://github.com/zq87xf5jyp-arch/YouNew.git` | GitHub Actions | Repository permissions and protected-branch policy | Release work is published on `codex/release-candidate-20260801` in draft PR #27; merge remains a separate reviewed operation |
| Apple App Store | iOS distribution entry | Apple listing ID `6782617312` | `https://apps.apple.com/app/id6782617312` | App Store Connect | Version 1.2 (7) is submitted and waiting for review; automatic immediate release after approval is enabled |
...
flowchart LR
    ...
    WEB --> SITES["OpenAI Sites / Cloudflare · younew.nl"]
    WEB --> STANDBY["Hostinger standby artifact"]
...
- No database migration or Edge Function source deployment is pending for this release; any later function change must be deployed from a reviewed release identity and smoke-tested independently.
...
- A fresh off-site age-encrypted logical SQL backup and isolated restore rehearsal require temporary database credentials that are not stored in the repository. Verification must decrypt only into the restore process, validate the manifest checksum and retain the JSON restore report.
- The iOS unit suite passed 479 tests; the Release simulator build, exact guide/place/organization deep links, Dark Mode with accessibility-extra-large and a 0-leak memgraph flow passed. Apple Distribution export and App Review submission are complete; installation of the exact 1.2 archive on the connected physical iPhone remains pending until the device is unlocked.
