...
| iOS app | Native YouNew product | `YouNew/` and `YouNew.xcodeproj` | Apple distribution | Apple signing and App Store controls | Version 1.1 is listed in the Netherlands App Store; candidate unit, simulator, exact deep-link and local device-archive gates pass, but no Apple Distribution export exists |
...
| GitHub | Version control and CI | `https://github.com/zq87xf5jyp-arch/YouNew.git` | GitHub Actions | Repository permissions and protected-branch policy | Release work is isolated on `codex/release-candidate-20260801`; the branch is not published, so its final SHA and CI remain release gates |
...
- The iOS unit suite passed 479 tests; the Release simulator build, exact guide/place/organization deep links, Dark Mode with accessibility-extra-large and a 0-leak memgraph flow passed. A development-signed arm64 device archive also completed, but an Apple Distribution archive/export and a physical-iPhone check remain external release gates.
