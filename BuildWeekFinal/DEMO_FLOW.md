...
Media-rights status updated: 2026-07-22. Shipped catalog `YouNew/Assets.xcassets`
passes its deterministic gate; screenshots and video remain a separate release
scope.
...
7. Соблюдать условия распространения из
   [`MEDIA_RIGHTS_FINAL.md`](MEDIA_RIGHTS_FINAL.md). В каталоге 170 governed
   assets и ноль unresolved записей; для видео сохранять обязательные credits и
...
| 12 | Нажать карточку текущего города **Amsterdam** и задержаться на hero/name. Если видны flag/coat tiles, использовать их только для информационной идентификации и не подразумевать одобрение муниципалитета. | Открывается детальная страница одного из пяти импортированных городов. Amsterdam hero входит в каталог с полной attribution metadata; городская символика имеет byte-exact Public Domain evidence. | `home.currentCity` → `city.detail.amsterdam`, `city.hero.image`, `city.relatedArticles.dashboard`; `MEDIA_RIGHTS_FINAL.md` |
...
- Shipped asset catalog имеет source-backed ledger и ноль unresolved записей.
  Это catalog-only PASS: screenshot/video/audio/public-site материалы требуют
  отдельной инвентаризации и не получают автоматического clearance.
...
- [ ] Все catalog assets проверены ledger/gate; сам записанный видеофайл и выбранные screenshots прошли отдельный release-media review.
- [ ] В video description/end card вставлены обязательные credits для всех
  воспроизведённых licensed photographs, включая license URLs и crop/resize notes.
- [ ] Screenshot 08 отдельно проверен как release-media artifact; видимая
  официальная символика не подаётся как муниципальное endorsement.
