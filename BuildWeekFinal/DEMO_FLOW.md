...
7. Использовать только bounded allowlist из
   [`MEDIA_RIGHTS_FINAL.md`](MEDIA_RIGHTS_FINAL.md). Не открывать карточки с
   одиннадцатью unresolved high-use raster assets.
...
| 12 | Нажать карточку текущего города **Amsterdam** и задержаться на hero/name. Не использовать flag/coat tiles как promotional artwork. | Открывается детальная страница одного из пяти импортированных городов. Amsterdam hero входит в bounded photo allowlist; официальная символика остаётся conditional. | `home.currentCity` → `city.detail.amsterdam`, `city.hero.image`, `city.relatedArticles.dashboard`; `MEDIA_RIGHTS_FINAL.md` |
...
- Контент и media rights не завершены для всего приложения. Для фотографий этого
  bounded flow подготовлен source-backed allowlist и обязательный credit block;
  это не доказывает repository-wide clearance.
...
- [ ] В video description/end card вставлены полные credits для Leiden, Haarlem и
  Amsterdam из `MEDIA_RIGHTS_FINAL.md`, включая license URLs и crop/resize notes.
- [ ] Screenshot 08 не используется как отдельное promotional artwork без owner
  acceptance официальной символики; при возможности записан hero/name-only кадр.
